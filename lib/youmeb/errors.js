...
errors.loadErrorConfigFile = function (file) {
  ...
  lines.forEach(function (line) {
    ...
    e['set' + level.key[0].toUpperCase() + level.key.substr(1)](last.replace(/\uffff/g, '.'), line[1], line[2]);
  });
...
errors.generate = function () {
  data.generate.apply(data, arguments);
};

errors.available = function () {
  data.available.apply(data, arguments);
};
...
errors.getCodes = function () {
  data.getCodes.apply(data, arguments);
};
